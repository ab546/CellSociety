...
public class Interface extends Application {
	...
	/**
	 * User Interface that allows interaction and displays simulations
	 * 
	...
	//Combines buttons, sliders, and scrollpane into a single scene for display
	private Scene setupGame(int width, int height, Paint background) {
		...
		// for(int i=0;i<new
		// cellsociety_team03.XML("data/test.xml").getAttributes().size();i++){
		int c = 0;
		...
		double speed=sliders.get(sliders.size()-1).getValue();
		//System.out.println(speed);
		KeyFrame frame = new KeyFrame(Duration.millis(10000/speed), e -> step());
		animation = new Timeline();
		animation.setCycleCount(Timeline.INDEFINITE);
	...
	private void getBoundaries() {
		...
		box.getItems().add("toroidal");
		box.getItems().add("regular");
		box.setLayoutX(700);
		box.setLayoutY(700);
		box.valueProperty().addListener((x, y, newValue) -> {
			myDefault.setToroidal(newValue.equals("toroidal"));
	...
	//initialize instance of selected game
	...
	//gets initial grid of cells from Default class
